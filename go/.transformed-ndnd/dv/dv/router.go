...
func NewRouter(config *config.Config, engine ndn.Engine) (*Router, error) {
	...
	if config.KeyChainUri == "insecure" {
		...
		kc, err := simNewKeyChain(config.KeyChainUri, store)
		if err != nil {
			return nil, err
		}
	...
	kc, err := simNewKeyChain(config.PrefixInsertionKeychainUri, insertionStore)
	if err != nil {
	...
	dv.advert = advertModule{
		dv:       dv,
		bootTime: uint64(_ndndsim.Now().Unix()),
		seq:      0,
...
func (dv *Router) register() (err error) {
	...
	err = dv.engine.AttachHandler(dv.config.AdvertisementSyncActivePrefix(),
		func(args ndn.InterestHandlerArgs) {
			_ndndsim.Go(func() {
				dv.advert.OnSyncInterest(args, true)
			})
		})
	if err != nil {
		return err
	...
	err = dv.engine.AttachHandler(dv.config.AdvertisementSyncPassivePrefix(),
		func(args ndn.InterestHandlerArgs) {
			_ndndsim.Go(func() {
				dv.advert.OnSyncInterest(args, false)
			})
		})
	if err != nil {
		return err
