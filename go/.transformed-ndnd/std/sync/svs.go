...
func NewSvSync(opts SvSyncOpts) *SvSync {
	...
	// Set default options
	if opts.BootTime == 0 {
		opts.BootTime = uint64(_ndndsim.Now().Unix())
	}
	...
	return &SvSync{
		...
		ticker:  newSimTicker(opts.PeriodicTimeout),
...
func (s *SvSync) main() {
	...
	s.faceCancel = s.o.Client.Engine().Face().OnUp(func() {
		_ndndsim.
			AfterFunc(100*time.Millisecond, s.sendSyncInterest)
	})
	defer s.faceCancel()
...
func (s *SvSync) onReceiveStateVector(args svSyncRecvSvArgs) {
	...
	if s.o.Passive && !s.passiveWillPersist.Swap(true) {
		_ndndsim.
			AfterFunc(5*time.Second, s.persistPassiveWires)
	}
...
func (s *SvSync) loadPassiveWires() {
	...
	_ndndsim.
		...
		AfterFunc(500*time.Millisecond, s.sendSyncInterest)
}
