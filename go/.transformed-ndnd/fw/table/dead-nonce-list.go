...
func (d *DeadNonceList) Insert(name enc.Name, nonce uint32) bool {
	...
	if !exists {
		d.list[hash] = true
		d.expirationQueue.Push(hash, _ndndsim.Now().Add(CfgDeadNonceListLifetime()).UnixNano())
	}
	return exists
}
...
func (d *DeadNonceList) RemoveExpiredEntries() {
	...
	defer d.mu.Unlock()
	evicted := 0
	for d.expirationQueue.Len() > 0 && d.expirationQueue.PeekPriority() < _ndndsim.Now().UnixNano() {
