...
func (p *PitCsTree) Update() {
	for p.pitExpiryQueue.Len() > 0 && p.pitExpiryQueue.PeekPriority() <= _ndndsim.Now().UnixNano() {
...
func (p *PitCsTree) FindMatchingDataFromCS(interest *defn.FwInterest) CsEntry {
	...
	if node != nil {
		if !interest.CanBePrefixV {
			if node.csEntry != nil &&
				(!interest.MustBeFreshV || _ndndsim.Now().Before(node.csEntry.staleTime)) {
...
func (p *pitCsTreeNode) findMatchingDataCSPrefix(interest *defn.FwInterest) CsEntry {
	if p.csEntry != nil && (!interest.MustBeFreshV || _ndndsim.Now().Before(p.csEntry.staleTime)) {
