...
const (
	...
	ruleInjectBierSimExtensions         // fw/bier/bier.go: per-node BIFT helpers
	...
	ruleBierSimBiftInTableAlgo          // dv/dv/table_algo.go: bier.Bift.BuildFromFib() → bier.SimBift().BuildFromFib()
...
func targetRewrites(outDir, simModule, phase string) []packageRewrite {
	...
	return []packageRewrite{
		// fw/fw: inject Thread sim helpers and route BIER replication through
		// the per-node simulated BIFT instead of the production global.
		pkg("fw/fw", map[string][]fileRule{
			"thread.go": {ruleInjectThreadSimExtensions, ruleBierSimBiftInTableAlgo},
			"bier.go":   {ruleBierSimBiftInTableAlgo},
		}),
		...
		pkg("fw/bier", map[string][]fileRule{
			"bier.go": {ruleInjectBierSimExtensions},
		}),
		// dv/table: apply global rewrites (time.Now→_ndndsim.Now,
		// time.Since→_ndndsim.Now().Sub, go→_ndndsim.Go) to all files.
		// Fixes lastSeen tracking and IsDead() in neighbor_table.go.
		...
		pkg("dv/dv", map[string][]fileRule{
			"table_algo.go":  {rulePostUpdateRibConvergenceHook, ruleBierSimBiftInTableAlgo},
...
func rewriteFile(fpath, simModule string, extraRules []fileRule) error {
	...
	for _, rule := range extraRules {
		...
		case ruleInjectBierSimExtensions:
			if applyBierSimExtensions(file, fset) {
				modified = true
				ndndsimUsed = true
			}
		...
		case ruleBierSimBiftInTableAlgo:
			modified = applyBierSimBiftInTableAlgo(file) || modified
